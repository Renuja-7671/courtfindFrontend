...
const CheckoutForm = () => {
  ...
  useEffect(() => {
    if (price) {
      setAmount(price);
      setAbsoluteAmount(parseInt(price));
    }
  }, [price]);
  ...
  const handleSubmit = async (e) => {
    ...
    const amountInUSD = price / conversionRate;
    ...
    try {
      ...
      if (result.error) {
        ...
        setTimeout(() => navigate(`/arenaPayment-success/${arenaId}/${price}`), 2000);
      }
    } catch (err) {
...
const arenaPaymentPage = () => (
  <Elements stripe={stripePromise}>
    <CheckoutForm />
  </Elements>
);

export default arenaPaymentPage;
